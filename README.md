...
def aplicar_mascara(image, mascara=[], const=[], nOutput="output.jpg", cmd=""):
    if image == None:
        image = Image.open("conejo.jpg")
...
def normalizar(image, max_values):
    ...
    for i in range(image.size[0]):
        for j in range(image.size[1]):
            (R, G, B) = pic[i, j]
            R = G = B = int((((float(R) / max_values[0]) * 255) + ((float(G) / max_values[1]) * 255) + (
            (float(B) / max_values[2]) * 255)) / 3)
            pic[i, j] = (R, G, B)
